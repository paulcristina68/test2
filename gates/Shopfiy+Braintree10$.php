    ...
    foreach ($paidUsers as $index => $line) {
        ...
        if ($userIdFromFile == $userId) {
            if ($userExpiryDate < $currentDate) {
                unset($paidUsers[$index]); // Remove expired user
                file_put_contents('Database/paid.txt', implode("\n", $paidUsers)); // Update file
                $freeUsers[] = $userId; // Add to free users list
                file_put_contents('Database/free.txt', implode("\n", $freeUsers)); // Update file
            } else {
                $rank = "PAID";
...
// Reset the rank and expiry date logic for free users if needed
if ($rank == "FREE") {
    $rank = "FREE";
    $expiryDate = "0";
}

// The $gcm variable needs to be properly initialized or removed if not used.
$gcm = ""; 
